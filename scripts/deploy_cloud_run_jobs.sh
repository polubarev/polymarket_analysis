...
function deploy_nightly_job() {
  gcloud run jobs deploy "${NIGHTLY_JOB_NAME}" \
    ...
    --task-timeout "7200s" \
    --max-retries "1" \
    --set-env-vars "OUTPUT_DIR=/tmp/polymarket-data,PIPELINE_PROFILE=ingest-nightly,GCS_OUTPUT_URI=${GCS_OUTPUT_URI},MAX_EVENTS=1000,WINDOW_DAYS=30,INTERVAL=1h,YES_ONLY_BINARY=false,PRICE_FETCH_WORKERS=2,RATE_WINDOW_S=10,FETCH_PRIORITY_MODE=history_first,HTTP_POOL_MAXSIZE=64,QUALITY_MIN_POINTS=24,QUALITY_MAX_MISSING_RATIO=0.9,QUALITY_MIN_PRICE_RANGE=0.005,INCREMENTAL_MODE=tail,INCREMENTAL_OVERLAP_POINTS=2,SKIP_INACTIVE_PRICED_ASSETS=true,SKIP_RAW_PRICE_FILES=true,NO_INCREMENTAL_PRICES=true,SNAPSHOT_ORDERBOOK=true,INGEST_VOLUME=true,RUN_SIGNALS=true,RUN_BACKTEST=true,GENERATE_CANDIDATES=true,INCLUDE_RESOLVED=true,SIGNAL_DEBUG=false,LOG_LEVEL=INFO"
}
...
function deploy_research_job() {
  gcloud run jobs deploy "${RESEARCH_JOB_NAME}" \
    ...
    --task-timeout "7200s" \
    --max-retries "1" \
    --set-env-vars "OUTPUT_DIR=/tmp/polymarket-data,PIPELINE_PROFILE=research-daily,GCS_OUTPUT_URI=${GCS_OUTPUT_URI},MAX_EVENTS=1000,WINDOW_DAYS=30,INTERVAL=1h,YES_ONLY_BINARY=false,PRICE_FETCH_WORKERS=2,RATE_WINDOW_S=10,FETCH_PRIORITY_MODE=history_first,HTTP_POOL_MAXSIZE=64,QUALITY_MIN_POINTS=24,QUALITY_MAX_MISSING_RATIO=0.9,QUALITY_MIN_PRICE_RANGE=0.005,INCREMENTAL_MODE=tail,INCREMENTAL_OVERLAP_POINTS=2,SKIP_INACTIVE_PRICED_ASSETS=true,SKIP_RAW_PRICE_FILES=true,NO_INCREMENTAL_PRICES=false,SNAPSHOT_ORDERBOOK=true,INGEST_VOLUME=true,RUN_SIGNALS=true,RUN_BACKTEST=true,GENERATE_CANDIDATES=true,INCLUDE_RESOLVED=true,SIGNAL_DEBUG=true,SIGNAL_DEBUG_LIMIT=20,LOG_LEVEL=INFO"
}
